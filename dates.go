...
func DateWithIncrement(increment string) time.Time {
	duration, _ := time.ParseDuration(increment)
...
func StartOfToday() time.Time {
	...
	return time.Date(reported.Year(), reported.Month(), reported.Day(), 0, 0, 0, 0, location)
}
...
func EndOfToday() time.Time {
	reported := time.Now()
	location, _ := time.LoadLocation("UTC")
	return time.Date(reported.Year(), reported.Month(), reported.Day(), 23, 59, 59, 0, location)
}
...
func DayBeforeYesterday() TimeWarp {
	return Today().DayBeforeYesterday()
}
